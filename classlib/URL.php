...
class ROCKETS_URL {
	...
	foreach($ar as $v) { // erase unwanted values
	    if(isset($output[$v])) $output[$v] = null;
	}
	return $output;
    }
	...
	if(!isset($result['host'])) return false;
	else return true;
    }
	...
	if($ar['non-www']==true) $host = str_replace("www.","",$host);
	if($ar['escape']==true) $host = str_replace(".","\.",$host);
	return $host;
    }
}
?>
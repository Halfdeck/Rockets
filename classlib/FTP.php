...
abstract class ROCKETS_FTP extends ROCKETS_ConfigurableObject {
	 ...
	 */
	protected function connect($ar = null)
	{
		$conn_id = ftp_connect($ar['server']) or trigger_error("Couldn't connect to {$ar['server']}", E_USER_ERROR);
		if (@ftp_login($conn_id, $ar['username'], $ar['password']) && self::$DEBUG)
	...
	public function download()
	{
		...
		foreach ($dirList as $file)
		{
			...
			if ($this->checkFilename($filename) == FALSE)
				continue;

			$path = $this->dir_downloadParent . $filename;

			if (self::$DEBUG)
				echo "Downloading {$filename} into {$path}<br>\r\n";

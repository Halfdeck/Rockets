...
class ROCKETS_ADMIN_File extends ROCKETS_ConfigurableObject {
	...
	if(isset($ar['deleteAfterMove'])) self::$deleteAfterMove = $ar['deleteAfterMove'];
	...
	if(!is_callable($callback)) trigger_error(__METHOD__ ." requires a callable function in \$callback", E_USER_ERROR);
	...
	while (!feof($fh)) {
	    $buffer = fgets($fh);
	    if(is_callable($callback)) call_user_func($callback, $buffer, $callbackData);
	}
	fclose($fh);
    }
    /**
	...
	$ar['path'] = $path;
	if(self::$DEBUG) echo "<h2>Deleting files from path: {$path}</h2>\r\n";
	...
	if(!is_dir($ar['path'].$ar['file'])) {
	    ...
	    if(self::$DEBUG) echo "deleted {$ar['path']}{$ar['file']}<br>";
	}
	else if(self::$DEBUG) echo "looks like {$ar['path']}{$ar['file']} is a directory, skipping...<br>";
    }
	...
	while ($file = readdir($dir_handle)) {
	   // if(self::$DEBUG && $c > $limit) break;
	    ...
	    else echo "callback not callable!<br>";
	...
	if(file_exists($ar["targetDir"].$ar['file'])) {
	    if(self::$DEBUG) echo "<strong>FILE EXISTS - {$ar["targetDir"]}{$ar['file']}<br></strong>\r\n";
